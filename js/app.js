document.addEventListener("DOMContentLoaded", () => {
  ...
  if (introOverlay && introCard) {
    ...
    if (introSkipBtn) {
      // Ngăn chặn sự kiện touchstart nổi bọt khi nhấp vào nút Bỏ qua giới thiệu
  ...
  // Load configuration and initialize the GIS map dynamically
  fetch("map data/compiled-config.json")
    .then((res) => res.json())
    .then((config) => {
      if (introEnterBtn) {
        introEnterBtn.setAttribute("title", "Khám phá Bản đồ số Xã " + config.commune_name);
      }
      initGISMap(config);
    })
  ...
  function initGISMap(config) {
    const defaultCenter = config.defaultCenter;
  ...
  // 4. Curated list of hamlets and their files dynamically generated
  const hamletNames = config.hamlets.map((h) => h.name);
  const hamletColors = config.hamlets.reduce((acc, h) => {
    acc[h.name] = h.color;
    return acc;
  }, {});
  ...
  const HAMLET_LABEL_CENTERS = config.hamlets.reduce((acc, h) => {
    acc[h.name] = h.center;
    return acc;
  }, {});
  ...
  function updateMapHUD(hamletName, mode) {
    ...
    if (hamletName && (mode === "hover" || mode === "selected")) {
      ...
      if (selectedHamletName) {
        ...
        hudText.innerText = "XÃ " + config.commune_name.toUpperCase();
  ...
  function selectHamlet(hamletName) {
    ...
    if (pathLayer && feature) {
      ...
      connectingLinesLayerGroup.clearLayers();
      const mergedHamletSources = config.mergedHamletSources;
  ...
  const oldHamletNames = config.oldHamlets.map((oh) => oh.name);
  ...
  oldHamletNames.forEach((name) => {
   ...
   // Thêm nhãn tên các ấp cũ của ấp 1 mới và ấp 2 mới (đã kéo gần vào nhãn mới 30% để khoảng cách gom tụ gọn gàng, đẹp mắt)
    const manualOldHamlets = config.manualOldHamlets; /*
  ...
  const communeProperties = config.communeProperties;
  ...
  function openCommuneSidebar() {
    ...
    // Header updates
    document.getElementById("commune-name").innerText = "XÃ " + config.commune_name.toUpperCase();
